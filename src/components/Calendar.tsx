...
export default function Calendar() {
  ...
  return (
    <section className={`${APP_CONFIG.calendar.backgroundColor} overflow-hidden`}>
      ...
      <div className="flex flex-row items-stretch p-4 sm:p-6 md:p-2 gap-4 sm:gap-6 md:gap-4">
        ...
        >
          <h2 className="font-serif text-sm sm:text-lg md:text-2xl text-[#E8E2D9] tracking-widest uppercase mb-4 md:mb-8 text-center" style={{ fontFamily: "'Playfair Display', serif" }}>
          ...
          <div className="w-full max-w-sm mx-auto">
            <div className="grid grid-cols-7 gap-1 md:gap-2 text-center mb-2 md:mb-6">
              {daysOfWeek.map(day => (
                <div key={day} className="text-[#D7B272] text-[10px] sm:text-xs md:text-sm font-serif tracking-wider">
                  {day}
                </div>
              ))}
            </div>
                    ...
                    {d ? (
                      <div className={`flex items-center justify-center rounded-full transition-all duration-300 ${
                        isWeddingDay 
                          ...
                          : "w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-[#A5ADB8]/30 hover:bg-[#D7B272]/20 text-[11px] sm:text-xs md:text-sm"
                      }`}>
                        {d}
                      </div>
                    ) : (
